...
router.get("/:themeId", validateUuidParam("themeId"), async (req: Request, res: Response) => {
...
router.patch("/:themeId", validateUuidParam("themeId"), validateBody(updateThemeSchema), async (req: Request, res: Response) => {
  const theme = await themesService.updateTheme(req.user!.sub, req.params.themeId, req.body);
  res.json(theme);
});

router.delete("/:themeId", validateUuidParam("themeId"), async (req: Request, res: Response) => {
  await themesService.deleteTheme(req.user!.sub, req.params.themeId);
  res.status(204).send();
});

router.post("/:themeId/install", validateUuidParam("themeId"), async (req: Request, res: Response) => {
