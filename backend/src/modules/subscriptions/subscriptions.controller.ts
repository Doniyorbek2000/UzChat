...
router.post("/channels/:conversationId", validateBody(createSubscriptionSchema), async (req: Request, res: Response) => {
  const sub = await subscriptionsService.subscribe(req.user!.sub, req.params.conversationId, req.body.tier ?? "basic");
  res.status(201).json(sub);
});

router.delete("/channels/:conversationId", async (req: Request, res: Response) => {
  await subscriptionsService.unsubscribe(req.user!.sub, req.params.conversationId);
  res.status(204).send();
});

router.get("/channels/:conversationId/subscribers", async (req: Request, res: Response) => {
  const subs = await subscriptionsService.getChannelSubscribers(req.params.conversationId);
  res.json(subs);
});

router.get("/channels/:conversationId/stats", async (req: Request, res: Response) => {
