...
router.patch("/:botId", validateBody(updateBotSchema), async (req: Request, res: Response) => {
  const bot = await botsService.update(req.user!.sub, req.params.botId, req.body);
  res.json(bot);
});

router.post("/:botId/regenerate-token", async (req: Request, res: Response) => {
  const result = await botsService.regenerateToken(req.user!.sub, req.params.botId);
  res.json(result);
});

router.post("/:botId/toggle-active", async (req: Request, res: Response) => {
  const bot = await botsService.toggleActive(req.user!.sub, req.params.botId);
  res.json(bot);
});

router.post("/:botId/commands", validateBody(addCommandSchema), async (req: Request, res: Response) => {
  const cmd = await botsService.addCommand(req.user!.sub, req.params.botId, req.body);
  res.status(201).json(cmd);
});

router.delete("/:botId/commands/:commandId", async (req: Request, res: Response) => {
